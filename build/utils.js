...
var pages = getFiles(config.build.viewPath + '/*/*.html');
// 所有页面html集合
exports.multiViews = function () {
    var plugins = [];
    for (var key in pages) {
        plugins.push(
            new HtmlWebpackPlugin({
                template: pages[key],
                filename: key + '.html',
                chunks: config.build.commonChunks.concat(key),
                layout: config.build.viewPath + 'layout.html'
            })
        );
    }
...
var entryJs = getFiles(config.build.viewPath + '/*/*.js');
// 所有页面入口js集合
exports.entries = entryJs;
// 所有页面名称集合
exports.viewNames = Object.keys(pages);
